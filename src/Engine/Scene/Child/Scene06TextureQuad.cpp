...
{
	void Scene06TextureQuad::Load(Renderer::Renderer& renderer) 
	{
	...
	bool Scene06TextureQuad::Update(float dt) 
	{
		const bool isRunning = ManageInput(inputState);
		if (inputState.IsPressed(Input::DirectionalKey::Left))
		{
		...
		if (inputState.IsPressed(Input::DirectionalKey::Right))
		{
	...
	void Scene06TextureQuad::Draw(Renderer::Renderer& renderer) 
	{
		renderer.Begin();
	...
	void Scene06TextureQuad::Unload(Renderer::Renderer& renderer) 
	{
